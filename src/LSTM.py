...
############### Modelo ############################## 
model = Sequential([
  # LSTM(20, activation=neuron,input_shape=[x_train.shape[1],x_train.shape[2]] ,return_sequences=True),
  Lambda(lambda x: tf.expand_dims(x, axis=-1),input_shape=[None]),
  LSTM(20, activation=neuron,return_sequences=True),
  LSTM(20, activation=neuron),
