...
stand_file = 'Datos_prueba/stand_01m.h5'
filled_file = 'Datos_prueba/filled_01m.h5'
...
def get_sensors_data(standarized=False):
    if standarized:
        radiation = h5py.File(stand_file, 'r')
    else:
        radiation = h5py.File(filled_file, 'r')
    features=[]
...
def window_data(input_data,timesteps=5,forecast=1,sensorToPredict='dh06'):
    X=[]
    y=[]
    for i in range(timesteps-1,len(input_data)-forecast):
        t=[]
        for j in range(-(timesteps-1),1):
            t.append(input_data[i+j,:].T)

        X.append(t)
        y.append(input_data[i+ forecast,sensors[sensorToPredict]])
...
def naive_forecast(input_data,timesteps=5,forecast=1,sensorToPredict='dh06'):
    prediction=[]
    for i in range(timesteps-1,len(input_data)-forecast):
...
def moving_average_forecast(input_data, timesteps=5,forecast=1,sensorToPredict='dh06'):
    prediction = []
    for i in range(timesteps-1,len(input_data)-forecast):
